...
test.describe("Founder customer UX: actionable incomplete states and Arabic RTL", () => {
  ...
  test("completes the incomplete-profitability review → exact Monthly fix → return journey on mobile", async ({ page }) => {
    ...
    await expect(page).toHaveURL(new RegExp(`${reviewHref!.replace(/[.*+?^\${}()|[\\]\\\\]/g, "\\\\    const reviewHref = await reviewJourneyLink.getAttribute("href");
...
test.describe("Founder customer UX: actionable incomplete states and Arabic RTL", () => {
  ...
  test("completes the incomplete-profitability review → exact Monthly fix → return journey on mobile", async ({ page }) => {
    ...
    expect(monthlyHref).toBe(
      `/businesses/${JOURNEY_BUSINESS_ID}/monthly?month=2026-04&origin=customer-profitability&return_month=2026-07`,
    );
    await monthlyFix.click();

    await expect(page).toHaveURL(new RegExp(`${monthlyHref!.replace(/[.*+?^\${}()|[\\]\\\\]/g, "\\\\    const monthlyHref = await monthlyFix.getAttribute("href");
...
test.describe("Founder customer UX: actionable incomplete states and Arabic RTL", () => {
  ...
  test("completes the incomplete-profitability review → exact Monthly fix → return journey on mobile", async ({ page }) => {
    ...
    expect(profitabilityHref).toBe(
      `/businesses/${JOURNEY_BUSINESS_ID}/customers?view=profitability&month=2026-07`,
    );
    await returnToProfitability.click();
    await expect(page).toHaveURL(new RegExp(`${profitabilityHref!.replace(/[.*+?^\${}()|[\\]\\\\]/g, "\\\\    const profitabilityHref = await returnToProfitability.getAttribute("href");
    expect(profitabilityHref).not.toBeNull();
    await page.goto(profitabilityHref!);
    await expect(page).toHaveURL(/\/auth\/e2e-lifetime-economics\?month=2026-07$/);")}import { expect, test, type Page, type Route } from "@playwright/test";
